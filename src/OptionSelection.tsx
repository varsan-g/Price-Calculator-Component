...
const OptionSelection: React.FC<OptionSelectionProps> = ({ title, options, onSelect }) => {
    return (
        <div>
            <h1 className="text-4xl font-bold mt-16 mb-24 text-white">{title}</h1>
            <div className="flex justify-center flex-wrap ">
                {options.map((option) => (
                    <div
                        key={option.value}
                        className="flex-auto min-w-0 max-w-full px-2 mb-4"
                        style={{ flexBasis: `calc(${100 / options.length}% - 1rem)` }}
                        onClick={() => onSelect(option.value, option.price)}
                    >
                        <div className="h-54 bg-white rounded-2xl flex flex-col items-center justify-center shadow-md cursor-pointer hover:bg-gray-100 text-center p-4 text-black">
                            <lord-icon
                                src={option.lordicon}
                                trigger="hover"
